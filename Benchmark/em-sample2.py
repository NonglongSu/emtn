...
def demo_2d():
    ...
    np.random.seed(2)
    m1, cov1 = [0,0], [[1, 1], [1, 1]] ## first gaussian
    data1 = np.random.multivariate_normal(m1, cov1, 90)
    
    m2, cov2 = [6, 13], [[1,1],[1,1]]#[[.5, -.5], [-.5, .1]] ## second gaussian
...
##    X = np.vstack((data1,np.vstack((data2))))
...
#    np.savetxt('sample.csv', X, fmt = "%.4f",  delimiter = ",")
    ...
    print ('Likelihoods:',params.log_likelihoods,
           'mu:',params.mu,
           'w:',params.w,
           'sigma:',params.Sigma)
    import pylab as plt    
